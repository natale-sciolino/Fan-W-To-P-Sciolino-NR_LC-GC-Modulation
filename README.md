...
Here's the phasic stim palatability experiment as an example:
```matlab
phasic_palatability.data_by_subject   % {1 × N subjects} cell array
phasic_palatability.subject_ids       % {1 × N subjects}
phasic_palatability.trial_type_names  % {1 × 4} tastant labels (e.g., {'S','N','C','Q'})
phasic_palatabilityt.time_vector       % [1 × T] time axis
```


`dataset.data_by_subject` contains taste-delivery-aligned data for each subject. It has dimensions:
